...
//            print(v1,v2)
   ...
   func createGetDepthDataByHttpStub() {
        ...
        stub(condition: isHost(XCTestBaseURLHost)) {
            req in
            print(req)
    ...
    func getDepthDataByHttp() {
        ...
        WFSAlamofire.getResponseDataInTestTarget(urlString,param) { response in
         ...
         //   self.getHttpLastId = dataDic!["lastUpdateId"] as? Int
            WLogSD("http请求getHttpLastId-\(self.getHttpLastId)")
    ...
    func setUPSocketAndConnect() {
        if tSocketRocke == nil {
            ...
            let urlString = XCTestBaseURL + XCTestUrlSuffix_Depth
...
extension BibiDepthManagerTest : WFSSocketRockeTestDelegate{
    ...
    public func sendData<T>(_ data: T) {
        ...
        //防止断开不及时，数据干扰
        WLogSD( "更新的Id比对--pushSocketLastId_u=\(self.pushSocketLastId_u)----pushSocketNewstId_U:\(self.pushSocketNewstId_U)------getHttpLastId:\(self.getHttpLastId)-----pushSocketNewstId_u:\(self.pushSocketNewstId_u)")
        //防止漏网的数据，造成误差
        if self.tSocketRocke._turnOffData{
            WLogSD("进入前有漏网数据----self.tSocketRocke._turnOffData")
            return
        }
        WLogSD( "进入后---pushSocketNewstId_U:\(self.pushSocketNewstId_U)-----pushSocketNewstId_u:\(self.pushSocketNewstId_u)")
        WLogSD("进来的数据前线程是----\(Thread.current)--\(self)")
        self.queue.async(execute: {
            self.semaphore.wait()
            //如果已经在重连，则快速释放已在队列中的任务
            if self.tSocketRocke._turnOffData{
                WLogSD("进入后有漏网数据----self.tSocketRocke._turnOffData")
                self.semaphore.signal()
                return
            }
            WLogSD("进来的数据后线程是----\(Thread.current)---\(self)")
			
            if self.firstConnctHttp {
                WLogSD("firstConnctHttp-----YES")
            ...
            WLogSD("asksArray---数量是-----\(asksArray?.count)")
            if asksArray != nil {
